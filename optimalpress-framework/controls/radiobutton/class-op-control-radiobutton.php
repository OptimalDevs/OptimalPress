	...
	protected function render_field( $value, $name ) {
		...
		<div class="input op-control-radiobutton">
			...
			<label>
				...
				<input <?php if( $checked ) echo 'checked'; ?> class="op-input <?php echo esc_attr( $this->field_classes ); ?><?php if( $checked ) echo " checked"; ?>" type="radio" name="<?php echo esc_attr( $name ); ?>" id="<?php echo esc_attr( $name ); ?>" value="<?php echo esc_attr( $item['value'] ); ?>" />
